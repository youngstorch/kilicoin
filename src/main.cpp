...
#if defined(NDEBUG)
# error "Kilicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4bac824a0228aefdeee4a54eb665fb32b72f2fabdecd949faae0f52b0d235671");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kilicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kilicoin Signed Message:\n";
...
{
    // Kilicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight <= 1000)
        // Premine: First 1K blocks@24M KCN will give 24 billion KCN
...
static const int64 nTargetSpacing = 1 * 60; // Kilicoin: 1 minutes

int64 GetTargetTimespan(int nHeight)
{
	return nHeight >= 97050 ? 10 * 60 // 6 minutes after 97050 fork
				: 20 * 60; // 20 minutes before
}
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kilicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Kilicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4bac824a0228aefdeee4a54eb665fb32b72f2fabdecd949faae0f52b0d235671");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "KiliCoin - Elia Minja";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("457984710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1494660271;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1133747;
        ...
        assert(block.hashMerkleRoot == uint256("0x0ecd28834dfa673bc6d6ce1afa9de8563f361a98f71110e684991f3e13ae0cfb"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Kilicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KilicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KilicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KilicoinMiner(CWallet *pwallet)
{
    printf("KilicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kilicoin-miner");
    ...
    try { loop {
        ...
        printf("Running KilicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KilicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KilicoinMiner, pwallet));
}
